(() => {
	...
	function a(t, e) {
		let a = function(t) {
				let e = 2166136261;
				for (let a = 0; a < t.length; a++) e ^= t.charCodeAt(a), e += (e << 1) + (e << 4) + (e << 7) + (e << 8) + (e << 24);
				return e >>> 0
			}(t),
			n = "";

		function r() {
			return a ^= a << 13, a ^= a >>> 17, a ^= a << 5, a >>> 0
		}
		for (; n.length < e;) {
			n += r().toString(16).padStart(8, "0")
		}
		return n.substring(0, e)
	}
	window.CAP_CUSTOM_WASM_URL || [`https://cdn.jsdmirror.com/npm/@cap.js/wasm@${t}/browser/cap_wasm.min.js`, `https://cdn.jsdmirror.com/npm/@cap.js/wasm@${t}/browser/cap_wasm_bg.wasm`].forEach(t => {
		const e = document.createElement("link");
		e.rel = "prefetch", e.href = t, e.as = t.endsWith(".wasm") ? "fetch" : "script", document.head.appendChild(e)
	});
	class n extends HTMLElement {
		...
		constructor() {
			super(), this.#s && this.#s.forEach((t, e) => {
				this.removeEventListener(e.slice(2), t)
			}), this.#s = new Map, this.boundHandleProgress = this.handleProgress.bind(this), this.boundHandleSolve = this.handleSolve.bind(this), this.boundHandleError = this.handleError.bind(this), this.boundHandleReset = this.handleReset.bind(this)
		}
		initialize() {
		...
		attributeChangedCallback(t, e, a) {
			if (t.startsWith("on")) {
				const e = t.slice(2),
					n = this.#s.get(t);
				if (n && this.removeEventListener(e, n), a) {
					const a = e => {
						const a = this.getAttribute(t);
						"function" == typeof window[a] && window[a].call(this, e)
					};
					this.#s.set(t, a), this.addEventListener(e, a)
				}
			}
			"data-cap-worker-count" === t && this.setWorkersCount(parseInt(a)), "data-cap-i18n-initial-state" === t && this.#r && this.#r?.querySelector("p")?.innerText && (this.#r.querySelector("p").innerText = this.getI18nText("initial-state", "Verify to continue"))
		}
		async connectedCallback() {
			this.#o = this, this.#n = this.attachShadow({
				mode: "open"
			}), this.#r = document.createElement("div"), this.createUI(), this.addEventListeners(), await this.initialize(), this.#r.removeAttribute("disabled");
			const t = this.getAttribute("data-cap-worker-count"),
				e = t ? parseInt(t, 10) : null;
			this.setWorkersCount(e || navigator.hardwareConcurrency || 8);
			const a = this.getAttribute("data-cap-hidden-field-name") || "cap-token";
			this.#o.innerHTML = `<input type="hidden" name="${a}">`
		}
		async solve() {
			if (!this.#i) try {
				this.#i = !0, this.updateUI("verifying", this.getI18nText("verifying-label", "Computing proof..."), !0), this.#r.setAttribute("aria-label", this.getI18nText("verifying-aria-label", "Computing proof of work, please wait")), this.dispatchEvent("progress", {
					progress: 0
				});
				try {
					let t = this.getAttribute("data-cap-api-endpoint");
					if (!t && window?.CAP_CUSTOM_FETCH) t = "/";
					else if (!t) throw new Error("Missing API endpoint. Either custom fetch or an API endpoint must be provided.");
					const {
						challenge: n,
						token: r
					} = await (await e(`${t}challenge`, {
						method: "POST"
					})).json();
					let o = n;
					if (!Array.isArray(o)) {
						...
						o = Array.from({
							length: n.c
						}, () => (t += 1, [a(`${r}${t}`, n.s), a(`${r}${t}d`, n.d)]))
					}
					const i = await this.solveChallenges(o),
						s = await (await e(`${t}redeem`, {
							method: "POST",
							body: JSON.stringify({
								token: r,
								solutions: i
					...
					if (this.dispatchEvent("progress", {
							progress: 100
						}), !s.success) throw new Error("Invalid solution");
					const c = this.getAttribute("data-cap-hidden-field-name") || "cap-token";
					this.querySelector(`input[name='${c}']`) && (this.querySelector(`input[name='${c}']`).value = s.token), this.dispatchEvent("solve", {
						token: s.token
					}), this.token = s.token, this.#e && clearTimeout(this.#e);
					const d = new Date(s.expires).getTime() - Date.now();
					return d > 0 && d < 864e5 ? this.#e = setTimeout(() => this.reset(), d) : this.error("Invalid expiration time"), this.#r.setAttribute("aria-label", this.getI18nText("verified-aria-label", "Proof of work completed, you may now continue")), {
						success: !0,
						token: this.token
					}
				} catch (t) {
					throw this.#r.setAttribute("aria-label", this.getI18nText("error-aria-label", "An error occurred during verification, please try again")), this.error(t.message), t
				}
			} finally {
				this.#i = !1
			}
		...
		async solveChallenges(e) {
			const a = e.length;
			let n = 0;
			const r = Array(this.#a).fill(null).map(() => {
					try {
						return new Worker(this.#t)
					} catch (t) {
						throw console.error("[cap] Failed to create worker:", t), new Error("Worker creation failed")
					}
				}),
				o = ([e, o], i) => new Promise((s, c) => {
					const d = r[i];
					if (!d) return void c(new Error("Worker not available"));
					const p = setTimeout(() => {
						try {
							d.terminate(), r[i] = new Worker(this.#t)
						} catch (t) {
							console.error("[cap] error terminating/recreating worker:", t)
						}
					...
					d.onmessage = ({
						...
						t.found && (clearTimeout(p), n++, this.dispatchEvent("progress", {
							progress: Math.round(n / a * 100)
						}), s(t.nonce))
					}, d.onerror = t => {
						clearTimeout(p), this.error(`Error in worker: ${t.message||t}`), c(t)
					}, d.postMessage({
						salt: e,
						target: o,
						wasmUrl: window.CAP_CUSTOM_WASM_URL || `https://cdn.jsdmirror.com/npm/@cap.js/wasm@${t}/browser/cap_wasm.min.js`
					})
				}),
				i = [];
			try {
				for (let t = 0; t < e.length; t += this.#a) {
					const a = e.slice(t, Math.min(t + this.#a, e.length)),
						n = await Promise.all(a.map((t, e) => o(t, e)));
					i.push(...n)
				}
			} finally {
				r.forEach(t => {
					if (t) try {
						...
						console.error("[cap] error terminating worker:", t)
					}
				})
			}
			return i
		...
		updateUI(t, e, a = !1) {
			if (!this.#r) return;
			this.#r.setAttribute("data-state", t), this.#r.querySelector("p").innerText = e, this.#r.setAttribute("aria-busy", "verifying" === t ? "true" : "false");
			const n = "verifying" === t ? "mixed" : "done" === t ? "true" : "false";
			this.#r.setAttribute("aria-checked", n), "error" === t ? this.#r.setAttribute("aria-invalid", "true") : this.#r.removeAttribute("aria-invalid"), a ? (this.#r.setAttribute("disabled", "true"), this.#r.setAttribute("aria-disabled", "true")) : (this.#r.removeAttribute("disabled"), this.#r.removeAttribute("aria-disabled"))
		}
		handleProgress(t) {
			if (!this.#r) return;
			const e = this.#r.querySelector("p"),
				a = this.#r.querySelector(".checkbox"),
				n = this.#r.querySelector(".sr-progress");
			e && a && (a.style.setProperty("--progress", `${t.detail.progress}%`), e.innerText = `${this.getI18nText("verifying-label","Verifying...")} ${t.detail.progress}%`), n && (n.setAttribute("aria-valuenow", String(t.detail.progress)), n.setAttribute("aria-valuetext", `${this.getI18nText("verifying-label","Verifying...")} ${t.detail.progress}%`)), this.executeAttributeCode("onprogress", t)
		}
		handleSolve(t) {
			this.updateUI("done", this.getI18nText("solved-label", "Verified"), !0), this.executeAttributeCode("onsolve", t)
		}
		handleError(t) {
			this.updateUI("error", this.getI18nText("error-label", "Error. Try again.")), this.executeAttributeCode("onerror", t)
		}
		handleReset(t) {
			this.updateUI("", this.getI18nText("initial-state", "Verify to continue")), this.executeAttributeCode("onreset", t)
		}
		executeAttributeCode(t, e) {
			const a = this.getAttribute(t);
			a && new Function("event", a).call(this, e)
		}
		error(t = "Unknown error") {
			console.error("[cap]", t), this.dispatchEvent("error", {
				isCap: !0,
				message: t
			})
		}
		dispatchEvent(t, e = {}) {
			...
			super.dispatchEvent(a)
		}
		reset() {
			this.#e && (clearTimeout(this.#e), this.#e = null), this.dispatchEvent("reset"), this.token = null;
			const t = this.getAttribute("data-cap-hidden-field-name") || "cap-token";
			this.querySelector(`input[name='${t}']`) && (this.querySelector(`input[name='${t}']`).value = "")
		}
		get tokenValue() {
			return this.token
		}
		disconnectedCallback() {
			this.removeEventListener("progress", this.boundHandleProgress), this.removeEventListener("solve", this.boundHandleSolve), this.removeEventListener("error", this.boundHandleError), this.removeEventListener("reset", this.boundHandleReset), this.#s.forEach((t, e) => {
				this.removeEventListener(e.slice(2), t)
			}), this.#s.clear(), this.#n && (this.#n.innerHTML = ""), this.reset(), this.cleanup()
		}
		cleanup() {
			this.#e && (clearTimeout(this.#e), this.#e = null), this.#t && (URL.revokeObjectURL(this.#t), this.#t = "")
		}
	}
	class r {
		constructor(t = {}, e) {
			const a = e || document.createElement("cap-widget");
			if (Object.entries(t).forEach(([t, e]) => {
					a.setAttribute(t, e)
				}), !t.apiEndpoint && !window?.CAP_CUSTOM_FETCH) throw a.remove(), new Error("Missing API endpoint. Either custom fetch or an API endpoint must be provided.");
			t.apiEndpoint && a.setAttribute("data-cap-api-endpoint", t.apiEndpoint), this.widget = a, this.solve = this.widget.solve.bind(this.widget), this.reset = this.widget.reset.bind(this.widget), this.addEventListener = this.widget.addEventListener.bind(this.widget), Object.defineProperty(this, "token", {
				get: () => a.token,
				configurable: !0,
				enumerable: !0
			}), e || (a.style.display = "none", document.documentElement.appendChild(a))
		}
	}
	window.Cap = r, customElements.get("cap-widget") || window?.CAP_DONT_SKIP_REDEFINE ? console.warn("[cap] the cap-widget element has already been defined, skipping re-defining it.\nto prevent this, set window.CAP_DONT_SKIP_REDEFINE to true") : customElements.define("cap-widget", n), "object" == typeof exports && "undefined" != typeof module ? module.exports = r : "function" == typeof define && define.amd && define([], () => r), "undefined" != typeof exports && (exports.default = r)
})();